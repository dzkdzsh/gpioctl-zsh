...
GPIO descriptor 申请成功后，core 会通过匹配 `hardware_key` 的 IOPAD provider
保存该 PAD 的受控字段，并把复用切换到该 PAD 对应的 GPIO 功能。保存值是不向
用户态暴露的 opaque 状态；当前飞腾 provider 仅保存和恢复 func、drive 与 bias，
不会覆盖寄存器中的无关位。provider 的 active/module 引用一直保持到租约释放，
避免租约期间卸载造成悬空回调。

显式 `release`、进程正常或异常退出、以及多线申请回滚都采用相同顺序：先应用
设备树 GPIO 安全态，再在 MMIO spinlock 下读改写恢复租约前的 IOPAD 受控字段，
读回核对后才释放 descriptor 和 provider 引用。恢复失败会记录错误，但不会因而
泄漏租约。这样既保证 GPIO 释放瞬间处于已声明安全态，也不会把临时 GPIO 复用和
电气参数永久留在 PAD 上。

## 释放 bias 与 IOPAD fallback
