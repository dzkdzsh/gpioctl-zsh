...
从 GitHub Release 下载 `Source code (tar.gz)`，不要先在 Windows 解压。进入下载
目录后复制下面整段 PowerShell。命令会询问板上实际登录用户名和 IP，不假设远端
账号使用任何固定名称：
...
在 Linux 板上解压可以保留 Shell 脚本的可执行权限。Windows 解压后再 `scp -r`
会使这些脚本变成 `0644`，从而让未修正的安装命令报“权限不够”。内核模块也必须
针对板上运行的内核构建，所以后续命令均在板子上执行。
...
若已经在 Windows 解压并传输了目录，先在板上的项目根目录恢复权限：
...
顶层 Makefile 对核心管理脚本另有 `sh` 调用兜底，但直接运行测试脚本仍要求执行位。
...
不改变 GPIO 电平。安装输出显示 `overlay=applied`、`list` 能列出控制器且设备
节点存在，即完成最小部署验证。
...
| `/etc/systemd/system/gpioctl-zsh.service` | 手动启动及可选开机自启动 unit |
| `/usr/local/sbin/gpioctl-zsh-autostart` | 自启动管理工具 |

安装脚本执行 `depmod`、重载 udev 规则，并只创建系统组 `gpio`，不会自动扩大成员。
它会启动当前会话的服务，但不会默认启用开机自启动。重启恢复和自启动配置见
[开机启动与重启恢复教程](autostart-guide-zsh.md)。
