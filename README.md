...
[deployment guide](docs/deployment-guide-zsh.md),
[boot/autostart guide](docs/autostart-guide-zsh.md), and
...
下面是一条已经从全新 `v0.1.3` 源码包实测到 `gpioctl_zsh list` 的完整路线。
...
再复制下面整段命令；前两行会分别询问板上的登录用户名和 IP 地址，项目不假设账号
必须叫 `zsh`、`root` 或其他固定名称：

```powershell
$BoardUser = Read-Host "板上登录用户名"
$BoardIp = Read-Host "板子 IP 地址"
scp .\gpioctl-zsh-0.1.3.tar.gz "${BoardUser}@${BoardIp}:~/"
...
```sh
tar -xzf ~/gpioctl-zsh-0.1.3.tar.gz -C ~
cd ~/gpioctl-zsh-0.1.3
...
- 安装输出包含 `gpioctl_zsh loaded; overlay=applied`；
...
回到刚才的 PowerShell 后，`$BoardUser` 和 `$BoardIp` 仍然有效，重新登录：
...
安装不是永久开机自启动。`sudo make install` 会让驱动在当前会话立即可用，但默认
保持 `gpioctl-zsh.service` disabled。板子重启后，手动恢复并验证只需要：

```sh
sudo systemctl start gpioctl-zsh.service
sudo systemctl status gpioctl-zsh.service --no-pager
gpioctl_zsh list
```

如果希望每次开机自动准备好 GPIO，执行一次：

```sh
sudo gpioctl-zsh-autostart enable
sudo gpioctl-zsh-autostart status
```

取消下次开机自启动不会中断当前 GPIO 客户端：

```sh
sudo gpioctl-zsh-autostart disable
```

需要同时停止当前服务时，再执行 `sudo systemctl stop gpioctl-zsh.service`。自启动只
加载驱动、应用 overlay 并创建设备节点，**不会恢复上次的 GPIO 高低电平**。完整的
启动顺序、日志、禁用、内核升级故障处理和本次开机验证方法见
[开机启动与重启恢复教程](docs/autostart-guide-zsh.md)。
...
- `sudo make install`：安装刚构建的模块、CLI、头文件、板级配置、udev 规则、
  overlay 和 systemd unit，执行 `depmod`，并为当前会话启动（但不 enable）服务。
- `sudo systemctl start gpioctl-zsh.service`：重启后手动应用/复用设备树 overlay，
  并按顺序加载驱动模块。
