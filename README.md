...
下面是一条完整路线，适用于“源码 Release 已在电脑上下载并解压，飞腾派可以通过
SSH 访问”的情况。内核模块必须针对板上正在运行的内核构建，因此这里把源码传到
板上后再编译，而不是在 Windows 电脑上直接运行 `make`。

### 1. 在电脑上把已解压目录传到板子

在 PowerShell 中进入解压目录的上一级。将 `<BOARD_IP>` 替换为板子的实际 IP；
`gpioctl-zsh-0.1.0` 是 GitHub `v0.1.0` 源码包解压后的目录名：
...
Linux/macOS 电脑使用相同命令，只需把路径写成 `./gpioctl-zsh-0.1.0`。如果下载的是
`.tar.gz` 且尚未解压，也可以先把压缩包 `scp` 到板子，再在板上执行
`tar -xzf gpioctl-zsh-0.1.0.tar.gz`。
...
- `sudo ./scripts/load_zsh.sh`：应用/复用设备树 overlay 并按顺序加载驱动模块。
